...
class GrokClient {
  ...
  private baseUrl: string = process.env.GROK_API_BASE_URL!;
  private defaultTimeoutMs: number = 60000;
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.name === "AbortError") {
          console.error(
            `Grok request timeout after ${timeoutMs}ms: ${endpoint}`
          );
