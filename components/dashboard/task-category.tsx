...
export function TaskCategory({ category, onCategoryChange, disabled = false }: TaskCategoryProps) {
  ...
  const categories: Record<TaskCategory, { label: string; icon: React.ReactNode; color: string }> = {
    assignment: { 
      label: "Assignment", 
...
export function TaskCategoryBadge({ category }: { category: TaskCategory }) {
  // Category configuration with student-focused labels and icons
  const categories: Record<TaskCategory, { label: string; icon: React.ReactNode; color: string; bgColor: string }> = {
    assignment: { 
      label: "Assignment", 
      icon: <FileText className="h-3 w-3" />,
      color: "text-blue-500",
  ...
  const { label, icon, color, bgColor } = categories[category];
