...
export function TaskCategory({
  ...
  if (!onCategoryChange || disabled) {
    return category ? (
      <Badge variant="outline" className={cn("gap-1", currentCategory.color)}>
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Badge
          variant="outline"
          className={cn(" gap-1 hover:bg-muted", currentCategory.color)}
        >
          {currentCategory.icon}
          <span>{currentCategory.label}</span>
        </Badge>
      ...
      <DropdownMenuContent align="end">
        ...
        {Object.entries(categoryConfig).map(([key, { label, icon, color }]) => (
          ...
          >
            <span className={cn("", color.split(" ")[0])}>{icon}</span>
            <span>{label}</span>
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
...
export function TaskCategoryBadge({ category }: { category: TaskCategory }) {
  ...
  return (
    <Badge
      variant="outline"
      className={cn(
        "px-2 py-0 h-5 text-xs font-normal flex items-center gap-1",
        color,
        bgColor
      )}
    >
